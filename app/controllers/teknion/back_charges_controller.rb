class Teknion::BackChargesController < ApplicationController
  ...
  @back_charge.issue_id = params['claim_issue_id']
    @related_claims_resp=Teknion::BackChargeIds.find(@back_charge.issue_id,session[:DealerCode])
  ...
  def create
    ...
    if params.key?('teknion_back_charge')
     attach=Attachments.new
      if !params['teknion_back_charge']['file_uploaded'].nil?
	results=attach.process_file(params['teknion_back_charge']['file_uploaded'],['Tekcare',params[:claim_issue_id]],'None')
      end
    end
      if backcharge_response.status == 201
        flash[:notice] = 'Back Charge was successfully updated.'
      else
        flash[:notice] = 'There was a problem creating your Journal.'
      end

      render inline: "<script> window.location.href = '#{teknion_claim_issue_path(params[:claim_issue_id], section: :back_charges, claim_id: params[:claim_id])}'; </script>"
 #   else
...
#      render :new, layout: false
  ...
  def update
    ...
    initial = Hash.new
    initial['document_date'] = Time.now if @back_charge.initial['document_date'].nil?
    initial['bc_type'] = params['teknion_back_charge']['bc_type_code']
    initial['bc_origin'] = 'REGULAR'
    initial['currency'] = params['teknion_back_charge']['currency']
    initial['estimate_from'] = 'INITINV'
    initial['estimated_amount'] = 555
    initial['description'] = "test test test"
    @back_charge.initial = initial

    shared_costs = Hash.new
    shared_costs['travel'] = 0
    shared_costs['material'] = 0
    shared_costs['rent'] = 0
    shared_costs['damage'] = 0
    shared_costs['other'] = 0
    shared_costs['regular_labour_hours'] = params['vc_reg_labour_hours']
    shared_costs['regular_labour_rate'] = params['vc_reg_labour_rate']
    shared_costs['regular_labour_type'] = params['vc_reg_labour_type']
    shared_costs['ot_labour_hours'] = params['vc_ot_labour_hours']
    shared_costs['ot_labour_rate'] = params['vc_ot_labour_rate']
    shared_costs['ot_labour_type'] = params['vc_ot_labour_type']
    @back_charge.shared_costs = shared_costs

    logger.ap @back_charge.save
    render nothing: true

    # TODO: uncomment and use the @back_charge.save here -CS
    #if @back_charge.save
      #flash[:notice] = 'Back Charge was successfully updated.'
      render inline: "<script> window.location.href = '#{teknion_claim_issue_path(@back_charge.claim_issue_id, section: :back_charge)}'; </script>"
    #else
      #render :edit, layout: false
    #end
  end
 def close_request_create
    ...
    if @clarification.valid?
      ...
      render inline: "<script> window.location.href = '#{teknion_claim_issue_path(@clarification.issue_id, section: :back_charges, claim_id: params[:claim_id])}'; </script>"
    else
  ...
  private
    def set_back_charge
      ...
      @back_charge = Teknion::BackCharge.find(params[:id], params[:dealer_code])
    end
