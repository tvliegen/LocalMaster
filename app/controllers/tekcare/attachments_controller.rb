class Tekcare::AttachmentsController < ApplicationController
    ...
    def new
      @attachment=Tekcare::Attachment.new
    ...
    if params.key?(:claim_id)
      complete_url="#{tekcare_claim_issue_path(params[:claim_issue_id], claim_id: params[:claim_id],section: 'attachments')}"
    else
      complete_url="#{tekcare_claim_issue_attachments_path(params[:claim_issue_id])}"
    end
