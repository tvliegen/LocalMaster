class Tekcare::SiteVisitsController < ApplicationController
  ...
  def update
    ...
    if @site_visit.valid?
     ...
     render inline: "<script> window.location.href = '#{tekcare_claim_issue_path(params[:issue_id], section: :site_visit, claim_id: params[:claim_id])}'; </script>"
    end

  private
    def set_site_visit
      @site_visit = Tekcare::SiteVisit.find(params[:id], session['DealerCode'])
    end

    def site_visit_params
      params.require(:tekcare_site_visit).permit(:claim_issue_id, :appointment_date, :appointment_time, :author, :confirmed_by, :site_visit_name)
    end
end
