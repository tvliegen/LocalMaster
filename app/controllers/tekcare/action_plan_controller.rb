class Tekcare::ActionPlanController < ApplicationController
  before_action :set_action_plan, only: [:show, :edit, :update, :destroy]

  def index
    @action_plan = Tekcare::ClaimIssue.find(params[:claim_issue_id]).action_plan
  ...
  def new
    @action_plan = Tekcare::ClaimIssue.find(params[:claim_issue_id]).build_action_plan
  end

  def create
    @action_plan = Tekcare::ClaimIssue.find(params[:claim_issue_id]).build_action_plan(action_plan_params)
  ...
  private
    def set_action_plan
     @action_plan ||= Tekcare::ActionPlan.find(params[:id])
    end
